<script lang="ts">
    import theme from "$lib/theme"
    import {onDestroy} from "svelte"

    export let placeholder = ""
    export let name = ""
    export let height = "3rem"
    export let outlineColor = "var(--outline-color)"
    export let outlineWidth = "1px"
    export let backgroundColor = "transparent"
    export let textColor = "var(--text-color)"
    export let textSize = "var(--text-smaller)"
    export let paddingLeftNoIcon = "1rem"
    export let paddingRightNoPassword = "1rem"
    export let initialValue: number | undefined = undefined
    export let passReference: (ref: HTMLInputElement) => void = (_) => {
    }
    export let icon: string | undefined = undefined
    export let autoFocus = false
    export let disabled = false
    export let onValueChange: (value: number | undefined) => void = (_) => {
    }
    export let iconColor = "var(--accent-color)"
    export let value = initialValue

    let reference: HTMLInputElement | undefined
    $: if (reference && autoFocus) reference.focus()
    $: if (reference) passReference(reference)

    $: onValueChange(value)

    let dark: boolean
    const themeUnsubscribe = theme.subscribe(value => dark = value === 'dark-theme')

    onDestroy(() => {
        themeUnsubscribe()
    })
</script>

<div class="icon-input-field"
     style="height: {height}; --outlineColor: {outlineColor}; --bgColor: {backgroundColor}; --textColor: {textColor}; --outlineWidth: {outlineWidth}">
    {#if icon}
        <i class="icon fi fi-br-{icon}" style="color: {iconColor}"></i>
    {/if}

    <input bind:this={reference} bind:value={value} type="number"
           name="{name}" placeholder="{placeholder}" min="0"
           style="font-size: {textSize}; padding-left: {icon ? '3rem' : paddingLeftNoIcon}; padding-right: {paddingRightNoPassword}; color-scheme: {dark ? 'dark' : 'light'};"
           disabled="{disabled}">
</div>

<style lang="scss">
  .icon-input-field {
    flex-shrink: 0;
    flex-grow: 0;
    position: relative;
    width: 100%;
    border-radius: 0.3rem;
    padding: 0 1rem;
    color: var(--textColor);
    isolation: isolate;
    border: var(--outlineWidth) solid var(--outlineColor);
    background-color: var(--bgColor);
    transition: background-color 0.3s ease-in-out, outline 0.3s ease-in-out;

    .icon {
      z-index: 1;
      position: absolute;
      top: 50%;
      left: 1.5rem;
      transform: translate(-50%, -50%);
    }

    input {
      border-radius: 0.25rem;
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      border: none;
      background: none;
      color: var(--textColor);
      accent-color: var(--accent-color);
      outline: 1px solid transparent;
      transition: outline-color 0.15s ease-in-out;

      &::placeholder {
        color: var(--label-color);
      }

      &:focus {
        outline-color: var(--accent-color);
      }

      &:-webkit-autofill {
        outline-color: var(--accent-color);
      }
    }

    .see-password-button {
      position: absolute;
      top: 50%;
      right: 0;
      transform: translate(0, -50%);
      display: flex;
      align-items: center;
      justify-content: center;
      height: 3rem;
      width: 3rem;
      border: none;
      outline: none;
      background: none;
      border-radius: 0.25rem;
      cursor: pointer;
    }
  }
</style>